...
import { FORBIDDEN, NOT_FOUND } from '../interceptors/default.interceptor'
...
export class AuthGuard implements ExpressMiddlewareInterface {
  async use(@Req() req: Request, @Res() res: Response, next: NextFunction) {
    authpal.authorizationMiddleware(req, res, async function () {
  ...
  async use(@Req() req: Request, @Res() res: Response, next: NextFunction) {
    authpal.authorizationMiddleware(req, res, async function () {
      if (!req.user.userid || !req.params.userId)
        throw new Error('Calling OwnerOrAdminGuard but something is missing...')
      let auth = await Users.findById(req.user.userid)
      if (auth) req.auth = auth
...
export class AdminGuard implements ExpressMiddlewareInterface {
  async use(@Req() req: Request, @Res() res: Response, next: NextFunction) {
    authpal.authorizationMiddleware(req, res, async function () {
      ...
      if (auth.admin) {
        return next()
      } else return next(new FORBIDDEN(`This resource is reserved for System Administrators`))
    })
  }
}
